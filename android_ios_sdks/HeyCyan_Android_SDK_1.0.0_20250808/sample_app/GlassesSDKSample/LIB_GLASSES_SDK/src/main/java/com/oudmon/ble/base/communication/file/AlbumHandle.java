        ...
        public void onReceivedData(String uuid, byte[] data) {
            if (data != null) {
                if ((data[0] & 0xff) == 0xbc && data[1] == 0x31) {
                    ...
                    //bc802f00e9bc 01 03 00 00 0c0f5cf48b2e007400780074000c790075002e007400780074001062600d5930528c542e00740078007400
                    int total = (ByteUtil.bytesToInt(Arrays.copyOfRange(data, 7, 8)));
                    int currIndex = (ByteUtil.bytesToInt(Arrays.copyOfRange(data, 8, 9)));
                    if (currIndex == 0) {
                        fileNames = new ArrayList<>();
                    }
                    if (total > 0) {
                        int offset = (ByteUtil.bytesToInt(Arrays.copyOfRange(data, 10, 11)));
                        parseEbookData(data, 11, offset, total, 0);
                    }else {
...
//            DeviceOperateManager.getInstance().execute(enableNotifyRequest);
    ...
    public void start(int packIndex) {
        byte[] data = new byte[]{0x02, (byte) packIndex};
        BleThreadManager.getInstance().addData(new BleDataBean(addHeader(0x80, data), mPackageLength));
    }

    public void deleteEbook(int position, String fileName) {
        try {
            ...
            byte[] data = new byte[]{0x02, (byte) position, (byte) deleteBytes.length};
            byte[] contactBytes = ByteUtil.concat(data, deleteBytes);
            BleThreadManager.getInstance().addData(new BleDataBean(addHeader(0x81, contactBytes), mPackageLength));
        } catch (Exception e) {
            e.printStackTrace();
        }
    }
...
//            byte[] fileNames = string2Unicode(fileName);
...
//         Log.i(TAG, "sendPocketToBle: mPackageLength: " + mPackageLength + ", bigPocket=" + DataTransferUtils.getHexString(bigPocket));
    ...
    public void initPath(Application application) {
        logPath = application.getExternalFilesDir("") + "/log/" + new DateUtil().getY_M_D() + "1_log.txt";
        logPath1 = application.getExternalFilesDir("") + "/log/" + new DateUtil().getY_M_D() + "2_log.txt";
    }
